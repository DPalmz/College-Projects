...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, hidden, num_heads, fcn_hidden, dropout=0.0):
        ...
        self.cross_attention = MultiHeadAttention(input_size=hidden, hidden=hidden, num_heads=num_heads, causal=False) #checkMultiHeadAttention

        self.self_att_layer_norm = nn.LayerNorm(hidden)
        self.cross_att_layer_norm = nn.LayerNorm(hidden)

        self.fcn = nn.Sequential(nn.Linear(hidden, fcn_hidden), nn.ReLU(), nn.Linear(fcn_hidden, hidden), nn.ReLU())
        self.fcn_layer_norm = nn.LayerNorm(hidden)
        self.dropout = nn.Dropout(dropout)
        # YOUR CODE ENDS HERE 
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        x = self.self_attention(residual)
        x = self.self_att_layer_norm(x + residual)
        residual = x
        x = self.cross_attention(q=x, kv=encoder_hidden_states, key_padding_mask=key_padding_mask)
...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.positional_emb = nn.Embedding(self.tgt_vocab_size, self.hidden)
        ...
        self.encoder_layers = nn.ModuleList([TransformerEncoderLayer(hidden=self.hidden, num_heads=self.num_heads, 
                                            fcn_hidden=self.fcn_hidden, dropout=self.dropout_rate) for _ in range( self.num_layers)])
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        embedding = self.encoder_embeddings(input_ids.to('cpu'))
        embedding = self._add_positions(embedding)
        for layer in self.encoder_layers:
            embedding = layer(embedding, key_padding_mask=key_padding_mask)
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        embedding = self.decoder_embeddings(decoder_input_ids.to('cpu'))
        embedding = self._add_positions(embedding)
        for layer in self.decoder_layers:
            embedding = layer(embedding, encoder_hidden_states, key_padding_mask)
        
        logits = self.out_proj(embedding)
        ## YOUR CODE ENDS HERE
        return logits
