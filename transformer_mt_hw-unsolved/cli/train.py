...
def preprocess_function(
    ...
    #The loop adds the start and end of sequence tokens to every sentence so the transformer knows where to start and stop
    # END OF YOUR ANSWER
    decoder_input_ids = []
    labels = []
    ...
    #For the model to learn to predict the next word
    # END OF YOUR ANSWER
    ...
    *,
    ...
    for batch in tqdm(dataloader, desc="Evaluation"):
        with torch.inference_mode():
            ...
            # YOUR ANSWER HERE (please limit your answer to 1-2 sentences):
            #model.forward calls the forward function, of which there several, one for each class. Generate is a self defined function
            # that assigns variables
...
def main():
    ...
    source_tokenizer = transformers.PreTrainedTokenizerFast(tokenizer_object=src_tokenizer_path)
    target_tokenizer = transformers.PreTrainedTokenizerFast(tokenizer_object=tgt_tokenizer_path)
    logging.info("tokenizers")
    # YOUR CODE ENDS HERE
    ...
    model = TransfomerEncoderDecoderModel(num_layers=args.num_layers, hidden=args.hidden_size, num_heads=args.num_heads, 
                                            ...
                                            src_vocab_size=len(source_tokenizer.vocab_files_names), 
                                            tgt_vocab_size=len(target_tokenizer.vocab_files_names), dropout=args.dropout_rate)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            if global_step % args.eval_every_steps == 0 or global_step == args.max_train_steps:
                eval_results, last_input_ids, decoded_preds, decoded_labels = evaluate_model(
                    model=model,
